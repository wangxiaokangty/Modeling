...
def save_csv(years, tourney, years_raw):
    ...
    for year in years:
        ...
        year.to_csv('../data/final_data/' + str(years_raw[year_num]) + '-' + tourney + '-points-final.csv')
        year_num += 1
...
def add_player_winner_sets(year, tourney):
    ...
    matches = pd.read_csv('../data/' + str(year) + '-' + tourney + '-matches.csv')
    points = pd.read_csv('../data/' + str(year) + '-' + tourney + '-points.csv')
    points = points[points.PointNumber.apply(lambda x: str(x).isnumeric())]
...
print('french')
wimbledon, wimb_list = merge_years(years3, 'wimbledon')
print('wimb')
...
save_csv(us_list, 'usopen', years1)
save_csv(french_list, 'frenchopen', years2)
save_csv(wimb_list, 'wimbledon', years3)
save_csv(aus_list, 'ausopen', years2)